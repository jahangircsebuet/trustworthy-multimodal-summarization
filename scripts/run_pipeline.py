...
from src.io.load import load_json
...
def main(args):
    recs = load_json(args.input)
    os.makedirs(args.out_dir, exist_ok=True)
    for rec in recs:
        rid = rec["id"]
        images = rec.get("images", []) or []
        ocr = run_ocr(images) if images else []
        asr = {}  # no ASR because we ignore video
...
#CUDA_VISIBLE_DEVICES=3 python -m scripts.run_pipeline --input datasets/dataset.json --out_dir outputs/results